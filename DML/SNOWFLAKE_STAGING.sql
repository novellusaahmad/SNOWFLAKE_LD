CREATE OR REPLACE PROCEDURE DATA_WAREHOUSE.STAGING_LD.LOAD_QA_DATA()
RETURNS STRING
LANGUAGE JAVASCRIPT
AS
$$
...
for (var i = 0; i < tables.length; i++) {
    ...
    var table_sql = `CREATE OR REPLACE TABLE DATA_WAREHOUSE.STAGING_LD.${tbl} (RAWDATA VARIANT)`;
    snowflake.execute({sqlText: table_sql});

    // Create JSON file format
    var ff_name = `FF_LD_JSON_${tbl}`;
    var ff_sql = `CREATE OR REPLACE FILE FORMAT DATA_WAREHOUSE.STAGING_LD.${ff_name}
                  TYPE=JSON
...
return "Loaded QA data for " + tables.length + " tables.";
$$;
